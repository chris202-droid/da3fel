...
class Recognize
{
	function RecognizeInscription($path, $image1, $image2)
	{
		if(!is_dir($path))
		{
			mkdir($path,0777,true);
			$result = exec("python Inscription.py $path $image2 $image1");
			return (int)$result;
		}else {return -1;}
		
	}

	function RecognizeEvaluation($path,$image)
	{
		if(is_dir($path))
		{
			$result = exec("python Evaluation.py $path $image");
